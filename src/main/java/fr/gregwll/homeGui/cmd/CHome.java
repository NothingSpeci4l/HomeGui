...
import fr.gregwll.homeGui.gui.HomesGui;
...
public class CHome implements CommandExecutor, TabCompleter {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        if (sender instanceof Player p) {
            if (args.length == 1) {
                ...
                TeleportUtils.teleportOrConfirm(p, loc, homeName);
            } else if (args.length == 0) {
                HomesGui.open(p, 0);
            } else {
                p.sendMessage(Constents.getPrefix() + "§f§lUsage: §7/home <homeName>");
            }
        }
        return true;
    }
